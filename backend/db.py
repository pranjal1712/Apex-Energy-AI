...
QDRANT_COLLECTION = os.getenv("QDRANT_COLLECTION_NAME", "nexus_collection")
...
def init_qdrant():
    ...
    try:
        if url and api_key:
            ...
            qdrant_client_instance = QdrantClient(url=url.strip(), api_key=api_key.strip(), timeout=20)
        else:
...
def ensure_collections():
    ...
    try:
        ...
        for idx in indexes:
            ...
            # Ensure Payload Indexes exist (Always try, ignore if already there)
            try:
