...
def chunk_text(text):
    from langchain_text_splitters import RecursiveCharacterTextSplitter # Modern import
...
def retrieve_and_rerank(query: str, user_id: str, doc_name: str = None):
    ...
    # 2. Search the Cloud/Local Collection
    try:
        # Using the newer query_points API which is present in your version
        response = db.qdrant_client_instance.query_points(
            collection_name=db.QDRANT_COLLECTION,
            query=query_vector,
            query_filter=models.Filter(must=must_conditions),
            limit=10
        )
        # Handle both list and object response styles
        res = response.points if hasattr(response, 'points') else response
    except Exception as e:
        print(f"[ERROR] Qdrant Search Critical Failure: {e}")
        res = []
